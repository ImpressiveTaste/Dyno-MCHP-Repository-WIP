...
case "set" then
    ...
    while %t do
        ...
        block_model.sim = list("x2c_TDSystemO2_"  + used_impl + "_C", 4);
...
define_style = "noLabel=0;displayedLabel=x<sub>k+1</sub> = A x<sub>k</sub> + B u<sub>k</sub><br>y<sub>k &nbsp &nbsp </sub> = C x<sub>k</sub>";
define_in_style = ["textOpacity=0";"textOpacity=0"];
define_out_style = ["textOpacity=0";"textOpacity=0"];
...
case "define" then
    ...
    model = scicos_model();
    used_impl = jinvoke(x2cPEdit, "getUsedImplementationName");
    model.sim = list("x2c_TDSystemO2_"  + used_impl + "_C", 4);
